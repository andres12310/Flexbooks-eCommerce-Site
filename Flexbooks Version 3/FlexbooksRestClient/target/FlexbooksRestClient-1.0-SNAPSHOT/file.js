...
function setData(){
    ...
    //    		}
                    console.log("about to print raw result");
                    var result = xhr.responseText;
                    console.log(result);  
                    result = result.split(";");
                    console.log("about to print split result");
                    console.log(result);
                    if (result){
  	...
  	xhr.open ("GET", "getItemPageData?id=" + ID);
  	xhr.send (null);	
}

function setRecentlyViewed() {
    console.log('recently viewed called');
    console.log(sessionStorage.getItem('itemsViewed'));
//    var ID = parseInt(sessionStorage.getItem('my_id'), 10); 
//        console.log(ID); 
    var bookInfo = '';
    if (sessionStorage.getItem('itemsViewed') != null){
        console.log('items viewed exists');
        var items = sessionStorage.getItem('itemsViewed');
        items.forEach(console.log('an item'))
    }
    ...
    //    		}
                    ...
                    if (result){
                        console.log('this should be working')
                            var [title, image, price, author, edition, type, page_count, publisher, language, isbn_10, isbn_13, dimensions, weight, quantity] = result;
                            document.getElementById('recent1').innerHTML = 
                            `<a id ='${id}' href='./itempage.html' onclick='setVar(${id})'>`+ 
    ...
    xhr.open ("GET", "getHomePageData");
    xhr.send ('Im testing stuff');
//    return `this is ${bookInfo}`;
